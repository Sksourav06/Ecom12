...
class FilterValueController extends Controller
{
     ...
     */
    public function store(FilterRequest $request,$filterId)
    {
        $filter = Filter::findOrFail($filterId);
        $this->filterValueService->store($request->validated(), $filterId);

        return redirect()->route('admin.filter_values.index', $filter->id)->with('success', 'Filter Value added successfully');
    }
     ...
     */
    public function update(Request $request, string $id,$filterId)
    {
        $filter = Filter::findOrFail($filterId);
        $this->filterValueService->update($request->validated(),$id, $filterId);
        return redirect()->route('admin.filter_values.index',$filter->id)->with('success', 'Filter Value updated successfully');
    }
    ...
    public function destroy(string $id)
    {
        ...
        return redirect()->route('admin.filter_values.index',$filterId)->with('success', 'Filter Value deleted successfully');
    }
}
